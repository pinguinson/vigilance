      ...
      private def checkConstructor(
        ...
        for (
          ...
          if (!usesParameter(paramName, constructorBody) &&
            ...
            context.warn(vparam.pos, UnusedMethodParameter.this, s"Unused constructor parameter (${vparam.name})")
          }
        }
      }
      ...
      override final def inspect(tree: Tree): Unit = {
        tree match {
          ...
          case d @ DefDef(mods, _, _, vparamss, _, rhs) =>
            vparamss
              .flatten
              .filter(param => !usesParameter(param.name.toString, rhs))
              .foreach { case ValDef(_, TermName(name), _, _) =>
                context.warn(tree.pos, UnusedMethodParameter.this, s"Unused method parameter: '$name'")
              }
