  ...
  def inspector(context: InspectionContext): Inspector = new Inspector(context) {
    override def traverser = new context.Traverser {
      ...
      /**
        ...
        */
      private def checkConstructor(vparamss: List[List[ValDef]], constructorBody: Tree, classBody: Tree): Unit = {
        for {
          vparams <- vparamss
          vparam <- vparams
        } yield {
        ...
        case d@DefDef(mods, _, _, vparamss, _, rhs) =>
          vparamss
            ...
            .foreach { case ValDef(_, TermName(name), _, _) =>
              context.warn(tree.pos, self, s"Unused method parameter: '$name'")
            }
      }
    }
  }
